...
init(autoreset=True)
import os
print(f"Created by **>>pangeranz<<** 2025/01/22")
def is_valid_country_code(phone_number):
    pattern = r"^\+\d{1,4}\d{10,12}$"  # Modified to allow 10 to 12 digits
    return re.match(pattern, phone_number) is not None
while True:
    user_input = input(Fore.CYAN + "Enter the phone number with country code (e.g. +628123456789): ")
    ...
    if is_valid_country_code(user_input):
        ...
        print(f"{Fore.RED}Please enter a valid number with a country code, like: +628123456789")
...
def generate_random_phone(country_selector):
    if country_selector == "ID": 
        return f"+62{random.choice([8])}{random.randint(10000000000, 99999999999)}"  
...
def send_requests(num_requests, delay):
    ...
    countries = ["ID", "US", "KR", "CN", "IN"]  # List of countries to choose from (ID=Indonesia)
    for item in ban:
